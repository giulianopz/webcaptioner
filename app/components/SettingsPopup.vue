<template>
  <div style="min-width:240px;max-height:80vh;overflow-y:auto">
    <div v-if="$store.state.user.signedIn">
      <img :src="$store.state.user.photoURL" v-if="$store.state.user.photoURL" class="rounded-circle float-left p-1 pr-2"
        style="max-width:50px" />
      <fa v-else icon="user-circle" style="font-size:2.5em" class="float-left mt-2 mr-2 text-muted" />
      ...
      <div class="pt-1" style="line-height:1.25rem">
        ...
        <b-button size="sm" style="font-size:.8rem" class="py-0 px-2 mt-1" variant="light" @click="signOut()">Sign
          Out</b-button>
      </div>
      <hr />
    </div>
    <a href="/captioner/settings/about" :target="this.$store.state.captioner.transcript.interim ||
    ...
    <!--     <nuxt-link to="/donate" :target="this.$store.state.captioner.transcript.interim ||
      this.$store.state.captioner.transcript.final
      ? '_blank'
      : '_self'
      " class="btn btn-light btn-sm btn-block mb-2 text-left">
    ...
    <!--     <nuxt-link to="/help" :target="this.$store.state.captioner.transcript.interim ||
        this.$store.state.captioner.transcript.final
        ? '_blank'
        : '_self'
      " class="btn btn-light btn-sm btn-block mb-2 text-left">
    ...
    <b-button v-if="$store.state.settings.exp.includes('srt')" :to="'/captioner/srt'" block variant="primary" size="sm"
      class="mb-2 text-left">
      <fa icon="flask" fixed-width class="mr-1" /> Export SRT
    </b-button>
    <hr />
    <b-button-group class="d-flex mb-2">
      <b-button :to="localePath('captioner-save-to-file')" variant="outline-secondary" v-b-tooltip.hover.top
        title="Save transcript">
        <fa icon="save" />
      </b-button>
      <b-button variant="outline-secondary" v-b-tooltip.hover.top :title="$t('navbar.menu.newWindow')"
        @click="$store.dispatch('START_DETACHED_MODE')">
        <fa icon="window-restore" />
      </b-button>
      <b-button variant="outline-danger" :to="localePath('captioner-clear')" v-b-tooltip.hover.top
    ...
    <div v-if="$store.state.user.signedIn === false" class="pt-1" style="line-height:1.25rem">
      <!-- Not signed in -->
      <hr />
      <b-button variant="light" block :to="localePath('captioner-sign-in')" @click="$emit('dismiss')">
